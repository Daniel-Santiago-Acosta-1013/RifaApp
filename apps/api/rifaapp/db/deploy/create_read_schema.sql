...
CREATE TABLE IF NOT EXISTS read.raffles (
    id uuid PRIMARY KEY,
    title text NOT NULL,
    description text,
...
CREATE INDEX IF NOT EXISTS raffles_read_status_idx ON read.raffles (status);

CREATE TABLE IF NOT EXISTS read.raffle_numbers (
...
CREATE INDEX IF NOT EXISTS raffle_numbers_read_status_idx ON read.raffle_numbers (raffle_id, status);
CREATE INDEX IF NOT EXISTS raffle_numbers_read_reserved_idx ON read.raffle_numbers (raffle_id, status, reserved_until);

CREATE TABLE IF NOT EXISTS read.purchases (
...
CREATE INDEX IF NOT EXISTS purchases_read_participant_idx ON read.purchases (participant_id);
...
SELECT p.id,
       ...
       p.payment_method,
       p.created_at
FROM write.purchases p
JOIN write.raffles r ON r.id = p.raffle_id
LEFT JOIN write.tickets t ON t.purchase_id = p.id
